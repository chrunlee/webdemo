...
// var factory = function($){
  	    ...
  	    if (filename !== null && typeof filename !== 'undefined') {
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    //added by lixun on 2017年5月19日 14:59:17,如果加载失败，默认加载中文配置文件 
                    if(filename.substring(filename.lastIndexOf('_')+1,filename.lastIndexOf('.')) != 'zh'){
                        var defaultFileName = filename.substring(0,filename.lastIndexOf('_'))+'_zh.properties';
                        loadAndParseFile(defaultFileName , settings , function(){
                            next();
                        });
                    }else{
                        if (jqXHR.status === 404) {
    ...
    //提供默认加载方式
    $.i18n.loadPro = function(name , path , lang, cb){
    	$.i18n.properties({
    		name : name,
    		path : path,
    		mode : 'map',
    		language : lang ,
    		callback : function(){
                byy.renderLocal('html',cb);
    			// $("[data-localize]").each(function() {
       //              var elem = $(this),
       //                  localizedValue = $.i18n.map[elem.data("localize")];
       //              if (elem.is("input[type=text]") || elem.is("input[type=password]") || elem.is("input[type=email]") || elem.is('textarea')) {
