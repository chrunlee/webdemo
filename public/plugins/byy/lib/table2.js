...
byy.define(['laytpl', 'page', 'win'], function(exports){
  ...
  ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.byy-table-page', ELEM_SORT = '.byy-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ,'{{# if(item2.type === "checkbox"){ }}' //复选框
                ,'<input type="checkbox" name="layTableCheckbox" class="byy-form-checkbox" title=" " byy-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>'
              ,'{{# } else { }}'
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              if(item3.type === 'checkbox'){
                return '<input type="checkbox" class="byy-form-checkbox" title=" " name="layTableCheckbox" byy-skin="primary" '+ function(){
    ...
    if(options.page){
      ...
      options.page.total = count; //更新总条数
      ...
      if(Math.round(elemCell.prop('scrollWidth')) > Math.round(elemCell.outerWidth())){
        that.tipsIndex = layer.tips([
          '<div class="layui-table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
            ...
            ,elemCell.html()
          ,'</div>'
          ,'<i class="layui-table-tips-c byyicon icon-close"></i>'
        ].join(''), elemCell[0], {
          tips: [3, '']
          ,time: -1
          ,anim: -1
